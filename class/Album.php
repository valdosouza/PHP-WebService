...
class Album extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['ALB_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_album', $dados, 'ALB_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_album', $dados, 'ALB_CODIGO');
		$this->query($sql);
		if($this->affected_rows()){
			return "1";
		}
	...
	public function delete($dados){
		$sql = "DELETE FROM tb_album WHERE ALB_CODIGO=".$dados['ALB_CODIGO'];
		$this->query($sql);
		if($this->affected_rows()){
			return "1";
		}
	...
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_album WHERE ALB_CODIGO=".$pk." LIMIT 1";
	...
	public function search($dados){
		...
		if(!empty($dados)){
			$sql .= " WHERE ";
			$sr = array();
			if(isset($dados['ALB_CODIGO'])){
				$sr[] = " ALB_CODIGO = ".$dados['ALB_CODIGO']." ";
			}
