...
class Estado extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['UFE_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_uf', $dados, 'UFE_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_uf', $dados, 'UFE_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_uf WHERE UFE_CODIGO=".$pk." LIMIT 1";
