...
class ContaBancaria extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['CTB_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_contabancaria', $dados, 'CTB_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_contabancaria', $dados, 'CTB_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function search($dados){
		$sql = "SELECT * FROM tb_contabancaria ";
		if(!empty($dados)){
			$sql .= " WHERE ";
			$sr = array();
	 ...
	 * @param Integer $CTB_CODIGO
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_contabancaria WHERE CTB_CODIGO=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT CTB_CODIGO FROM tb_contabancaria WHERE CTB_CODDKP=".$dados['CTB_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['CTB_CODIGO'])){return 0;}
		return $d['CTB_CODIGO'];
	}
