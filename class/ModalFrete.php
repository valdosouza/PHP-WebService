...
class ModalFrete extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['MDF_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_modal_frete', $dados, 'MDF_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_modal_frete', $dados, 'MDF_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function search($dados){
		$sql = "SELECT * FROM tb_modal_frete ";
		if(!empty($dados)){
			$sql .= " WHERE ";
			$sr = array();
	 ...
	 * @param Integer $MDF_CODIGO
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_modal_frete WHERE MDF_CODIGO=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT MDF_CODIGO FROM tb_modal_frete WHERE MDF_CODDKP=".$dados['MDF_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['MDF_CODIGO'])){return 0;}
		return $d['MDF_CODIGO'];
	}
