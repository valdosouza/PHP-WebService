...
class Banco extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['BCO_CODETD'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_banco', $dados, 'BCO_CODETD');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_banco', $dados, 'BCO_CODETD');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	 ...
	 * @param Integer $BCO_CODETD
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_banco WHERE BCO_CODETD=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT BCO_CODETD FROM tb_banco WHERE BCO_CODDKP=".$dados['BCO_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['BCO_CODETD'])){return 0;}
		return $d['BCO_CODETD'];
	}
