...
class FormaPagto extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['FPG_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_formapagto', $dados, 'FPG_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_formapagto', $dados, 'FPG_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	 ...
	 * @param Integer $FPG_CODIGO
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_formapagto WHERE FPG_CODIGO=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT FPG_CODIGO FROM tb_formapagto WHERE FPG_CODDKP=".$dados['FPG_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['FPG_CODIGO'])){return 0;}
		return $d['FPG_CODIGO'];
	}
