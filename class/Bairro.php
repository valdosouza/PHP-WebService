...
class Bairro extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['BRR_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_bairro', $dados, 'BRR_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_bairro', $dados, 'BRR_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_bairro WHERE BRR_CODIGO=".$pk." LIMIT 1";
