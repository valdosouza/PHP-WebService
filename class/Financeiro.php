...
class Financeiro extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['FIN_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_financeiro', $dados, 'FIN_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_financeiro', $dados, 'FIN_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	 ...
	 * @param Integer $FIN_CODIGO
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_financeiro WHERE FIN_CODIGO=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT FIN_CODIGO FROM tb_financeiro WHERE FIN_CODDKP=".$dados['FIN_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['FIN_CODIGO'])){return 0;}
		return $d['FIN_CODIGO'];
	}
