...
class ObsNfe extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['OBN_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_obs_nfe', $dados, 'OBN_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_obs_nfe', $dados, 'OBN_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	 ...
	 * @param Integer $OBN_CODIGO
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_obs_nfe WHERE OBN_CODIGO=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT OBN_CODIGO FROM tb_obs_nfe WHERE OBN_CODDKP=".$dados['OBN_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['OBN_CODIGO'])){return 0;}
		return $d['OBN_CODIGO'];
	}
