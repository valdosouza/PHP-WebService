...
class Natureza extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['NAT_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_natureza', $dados, 'NAT_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_natureza', $dados, 'NAT_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	 ...
	 * @param Integer $NAT_CODIGO
	 * @return Boolean true|false 
	 */
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_natureza WHERE NAT_CODIGO=".$pk." LIMIT 1";
	...
	public function pesquisaCDKP($dados){
		$sql = "SELECT NAT_CODIGO FROM tb_natureza WHERE NAT_CODDKP=".$dados['NAT_CODDKP'];
		$d = $this->query_return_one($sql);
		if(empty($d['NAT_CODIGO'])){return 0;}
		return $d['NAT_CODIGO'];
	}
