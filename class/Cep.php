...
class Cep extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['CEP_NUMERO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_cep', $dados, 'CEP_NUMERO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_cep', $dados, 'CEP_NUMERO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_cep WHERE CEP_NUMERO='".$pk."' LIMIT 1";
