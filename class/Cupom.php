...
class Cupom extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['CPM_TOKEN'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_cupom_promocional', $dados, 'CPM_TOKEN');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_cupom_promocional', $dados, 'CPM_TOKEN');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function delete($dados){
		$sql = "DELETE FROM 
					tb_cupom_promocional 
				WHERE 
					CPM_TOKEN=".$dados['CPM_TOKEN'];
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return 1;
	...
	public function search($dados){
		$sql = "SELECT * FROM tb_cupom_promocional ";
		if(!empty($dados)){
			$sql .= " WHERE ";
			$sr = array();
	...
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_cupom_promocional WHERE CPM_TOKEN=".$pk." LIMIT 1";
