...
class Frete extends Mysql implements InterfaceWebService{
	...
	public function insert($dados){
		if($this->hasThisPk(@$dados['FRT_CODIGO'])){
			return $this->update($dados);
		}
		$sql = $this->geraSql('insert', 'tb_frete', $dados, 'FRT_CODIGO');
		$this->query($sql);
	...
	public function update($dados){
		$sql = $this->geraSql('update', 'tb_frete', $dados, 'FRT_CODIGO');
		$this->query($sql);
		$r = $this->affected_rows();
		if($r>0){
			return "1";
	...
	public function hasThisPk($pk){
		$sql = "SELECT 1 FROM tb_frete WHERE FRT_CODIGO='".$pk."' LIMIT 1";
